# ==============================
# Corporate Default Risk Scoring
# Bank-grade single page (Streamlit)
# ==============================
...
# ==== Your utils (đã có sẵn trong repo) ====
...
# ---- Plotly wrapper: no warnings, unique keys ----
...
if hasattr(model, "predict_proba"):
    pd_model = float(model.predict_proba(X_base)[:, 1][0])
else:
    pd_model = float(model.predict(X_base)[0])

# Bank-grade post-model adjustments
PD_CFG = {
    ...
    "pd_floor": {"UPCOM": 0.12, "HNX": 0.07, "HOSE": 0.03, "HSX": 0.03, "__default__": 0.05},
    "pd_cap":   {"default": 0.98}
}

def _get(series, keys, default=np.nan):
    for k in keys:
...
risk_intensity = 1.0
if flags["dta_hi"]:       risk_intensity += 0.25
if flags["dte_hi"]:       risk_intensity += 0.20
if flags["netde_hi"]:     risk_intensity += 0.15
if flags["cr_low"]:       risk_intensity += 0.15
if flags["qr_low"]:       risk_intensity += 0.10
if flags["roa_neg"]:      risk_intensity += 0.20
if flags["roe_neg"]:      risk_intensity += 0.10
if flags["npm_neg"]:      risk_intensity += 0.10
if flags["rev_cagr_neg"] :risk_intensity += 0.10
if flags["assets_q40"]:   risk_intensity += 0.10
if flags["revenue_q40"]:  risk_intensity += 0.05
...
adj *= risk_intensity
pd_final = float(np.clip(_sigmoid(logit0 + adj),
                         PD_CFG["pd_floor"].get(exchange, PD_CFG["pd_floor"]["__default__"]),
                         PD_CFG["pd_cap"]["default"]))
...
with c3:
    st.markdown(
        ...
        f"Floor/Cap: {PD_CFG['pd_floor'].get(exchange, PD_CFG['pd_floor']['__default__']):.0%}/{PD_CFG['pd_cap']['default']:.0%} • "
        f"Exchange: {exchange or '-'}</span>", unsafe_allow_html=True
    )
...
if shap_df.empty:
    ...
    shap_col = _pick_col(shap_df, ["SHAP","shap","shap_value","impact","value"])
...
# Severity tự động theo hồ sơ DN (từ risk_intensity) & bucket
def firm_sensitivity_auto():
    sens = 1.0 + (risk_intensity - 1.0)
    if sector_bucket == "Real Estate": sens *= 1.15
    if exchange == "UPCOM": sens *= 1.10
    return float(np.clip(sens, 0.9, 3.0))
FIRM_SENS = firm_sensitivity_auto()

# Crisis libraries (đã loại Interest_Coverage / EBITDA_to_Interest)
SECTOR_CRISES = {
    "Materials": [
    ...
    "Consumer Staples": [
        ...
        ("Supply Chain", {"Inventory_Turnover":0.92,"Receivables_Turnover":0.95})
    ],
    "Industrials": [
...
def _scale_map(base_map: dict, k: float) -> dict:
    ...
    for feat, mult in base_map.items():
        out[feat] = float(np.clip(1.0 + (mult - 1.0) * k, 0.5, 1.9))
    return out

base_sev = 1.15
if sector_bucket in ["Real Estate","Materials","Consumer Discretionary"]: base_sev += 0.10
if exchange == "UPCOM": base_sev += 0.10
SEVERITY = float(np.clip(base_sev * FIRM_SENS, 1.0, 2.5))

st.caption(f"Sector (raw): {sector_raw or '-'} → Bucket: **{sector_bucket}** • Firm sensitivity: ×{FIRM_SENS:.2f} • Severity used: **×{SEVERITY:.2f}**")
...
for name, fmap in sector_list:
    ...
    pd_c = _pd_from_row(model, apply_feature_multipliers_alias(X_base, fmap_scaled))
...
with k2: st.metric("Max PD under crises", f"{max(df_sector['PD'].max() if not df_sector.empty else 0.0, df_sys['PD'].max() if not df_sys.empty else 0.0):.2%}")
