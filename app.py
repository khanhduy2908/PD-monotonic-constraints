...
# ==== Utils (đã có sẵn trong repo của bạn) ====
...
def fmt_ratio(x):
    ...
    # nếu là tỷ lệ thực (|x| <= 1.5) thì in %
...
def model_align_row(row: pd.Series, model, fallbacks: list) -> pd.DataFrame:
    """map row -> 1xN đúng thứ tự feature của model; thêm cột thiếu = 0; bỏ cột thừa"""
...
def load_raw_and_features():
    ...
    # RAW for overview (để số liệu đúng, không log)
...
st.caption("English UI • Single page • LightGBM scoring • SHAP • Sector-specific & Systemic stress • Monte Carlo CVaR • PSI drift")
...
# candidate features: all numeric in feats_df minus id/label
...
# chọn hàng cho model (từ feats_df)
...
ex_intensity = exchange_intensity(exchange)

# Lấy số liệu RAW (đẹp, đúng)
...
# Ratios from RAW (clean & realistic)
...
ebitda_to_interest = np.nan  # có thể bổ sung nếu bạn có Depreciation trong raw
...
stats_all = compute_feature_stats(feats_df, features_order)
...
if ref_explicit is None:
    st.info("Training reference not found. Using entire feature dataset as baseline for PSI.")
    ref_df = feats_df[common_cols].copy()
else:
    ...
    feats_df = feats_df[common_cols_psi]
    common_cols = common_cols_psi

ref_df = safe_df(ref_df)
score_df = safe_df(feats_df[common_cols])
...
bullets = [
    ...
    f"PD: {pd_base:.2%} → Policy band: {classify_pd(pd_base, thr)}.",
    ...
    "PSI monitors feature shift vs. training baseline. Any 'Shift' requires review."
]
