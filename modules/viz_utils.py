...
def plot_default_distribution_year(df: pd.DataFrame):
    ...
    fig = px.bar(summary, x="Year", y="Count", color="Default",
                 ...
                 title="Default vs Non-Default by Year (Selected Subset)")
    fig.update_layout(legend_title_text="Default", bargap=0.2)
    return fig
...
def plot_default_rate_by_sector(df: pd.DataFrame):
    ...
    fig.update_layout(xaxis_tickangle=-40, coloraxis_showscale=False, bargap=0.3)
    return fig

def plot_probability_histogram(scored_df: pd.DataFrame):
    if "Default_Proba" not in scored_df.columns or len(scored_df) == 0:
        return go.Figure()
    fig = px.histogram(scored_df, x="Default_Proba", nbins=20,
...
def plot_pd_line_forecast(forecast_df: pd.DataFrame, threshold: float = None):
    ...
    if threshold is not None:
        fig.add_hline(y=threshold, line_dash="dash", line_color="#e63946", annotation_text=f"Threshold {threshold:.2f}")
    return fig
...
def plot_pd_risk_bucket_bar(forecast_df: pd.DataFrame, threshold: float):
    ...
    def bucket(p):
        ...
        return "Low Risk"
    df["Risk_Bucket"] = df["Default_Proba"].apply(bucket)
